...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    authors: [{ name: content.brand.name }], creator: content.brand.name, publisher: content.brand.name,
    alternates: { canonical: "/" },
    openGraph: { type: "website", locale: "id_ID", url: SITE_URL, siteName: content.brand.name, title, description, images: [{ url: socialImage, width: 1200, height: 630, alt: content.brand.name }] },
    twitter: { card: "summary_large_image", title, description, images: [socialImage] },
    icons: content.brand.logoUrl ? { icon: content.brand.logoUrl, apple: content.brand.logoUrl } : undefined,
    robots: { index: true, follow: true }, category: "education",
  };
}

export default async function RootLayout({ children }: { children: React.ReactNode }) {
  const content = await getSiteContent();
  const structuredData = { "@context": "https://schema.org", "@type": "EducationalOrganization", name: content.brand.name, alternateName: content.brand.shortName, url: SITE_URL, logo: content.brand.logoUrl || undefined, description: content.brand.tagline, address: { "@type": "PostalAddress", streetAddress: content.contact.address, addressLocality: "Plumbon", addressRegion: "Jawa Barat", addressCountry: "ID" }, sameAs: [content.social.instagram, content.social.facebook].filter(Boolean) };
